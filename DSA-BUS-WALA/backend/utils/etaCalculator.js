...
/**
 * Compute how many stops remain between the bus's current stop index and a
 * target stop. Returns null when the target is already passed/unknown.
 * @param {Array} orderedStops - route stops sorted by seq
 * @param {number} currentStopIndex - bus's current stop index
 * @param {number|string} targetSeq - target stop sequence
 * @returns {number|null}
 */
const stopsAway = (orderedStops, currentStopIndex, targetSeq) => {
  if (!Array.isArray(orderedStops) || !orderedStops.length) return null;
  const targetIdx = orderedStops.findIndex(
    (s) => String(s.seq ?? s.sequence) === String(targetSeq)
  );
  if (targetIdx === -1) return null;
  const currentIdx = Math.max(currentStopIndex || 0, 0);
  const away = targetIdx - currentIdx;
  return away >= 0 ? away : null;
};

/**
 * Resolve a scheduled ETA (minutes) from the Schedule collection for a route.
 * Falls back to a static default when no schedule exists.
 * The result is distinctly labelled so UI shows "scheduled" vs live GPS ETA.
 * @param {Object} route - route document
 * @param {number|null} targetSeq - target stop sequence
 * @returns {Promise<{ etaMs: number|null, source: 'scheduled'|null }>}
 */
const computeScheduledEta = async (route, targetSeq) => {
  if (!route) return { etaMs: null, source: null };
  try {
    const Schedule = require('../models/Schedule');
    const schedule = await Schedule.findOne({ route: route._id, isActive: true })
      .sort({ departureTime: 1 })
      .lean();
    if (!schedule) return { etaMs: null, source: null };

    const [hh, mm] = schedule.departureTime.split(':').map(Number);
    const departure = new Date();
    departure.setHours(hh, mm, 0, 0);
    // If departure is in the past today, push to tomorrow
    if (departure < new Date()) departure.setDate(departure.getDate() + 1);

    const now = Date.now();
    const etaMs = Math.max(0, departure.getTime() - now);
    return { etaMs, source: 'scheduled' };
  } catch (err) {
    return { etaMs: null, source: null };
  }
};
