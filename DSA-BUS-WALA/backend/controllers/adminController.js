...
const { importDataset } = require('../utils/datasetImporter');
 ...
 * Accepts an uploaded Excel/CSV FETRI dataset and imports routes/stops/buses/schedules.
 */
const importDatasetHandler = async (req, res) => {
  try {
    if (!req.file) {
      return res.status(400).json({ message: 'A dataset file (.xlsx/.xls/.csv) is required' });
    }

    const source = req.body?.source === 'admin' ? 'admin' : 'fetri';
    const result = await importDataset(req.file.buffer, source);
    ...
    console.error('importDatasetHandler error:', error);
    res.status(500).json({ message: 'Failed to import dataset', error: error.message });
  }
};

/**
 * GET /api/admin/export-live-gps
 *
 * Exports live GPS tracking history to an Excel (.xlsx) file for a selected
 * date, bus or route. Supports token-in-query for direct browser downloads.
 *
 * Query params:
 *   date   (YYYY-MM-DD, optional, defaults to today)
 *   busId  (optional)
 *   routeId (optional)
 *   token  (optional — for browser download without Authorization header)
 */
const exportLiveGpsExcel = async (req, res) => {
  try {
    const { date, busId, routeId, token } = req.query;
    ...
    // Build time range. Default to today.
    const now = new Date();
    ...
    const records = await LocationEvent.find(query)
      .populate('bus', 'name numberPlate')
      .populate('route', 'name')
      .populate('trip', 'startedAt endedAt')
      .sort({ timestamp: 1 })
      .lean();

    const rows = records.map(r => ({
      eventId: r.eventId,
      busId: r.bus?._id?.toString() || r.bus,
      busName: r.bus?.name || '',
      numberPlate: r.bus?.numberPlate || '',
      tripId: r.trip?._id?.toString() || r.trip,
      routeId: r.route?._id?.toString() || r.route,
    ...
    const sheetName = date || 'live-gps';
    XLSX.utils.book_append_sheet(wb, ws, sheetName);
    const buffer = XLSX.write(wb, { type: 'buffer', bookType: 'xlsx' });

    const filename = `live-gps-${date || now.toISOString().slice(0, 10)}${busId ? `-bus-${busId}` : ''}${routeId ? `-route-${routeId}` : ''}.xlsx`;

    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.send(buffer);
  } catch (error) {
    console.error('exportLiveGpsExcel error:', error);
    res.status(500).json({ message: 'Failed to export live GPS history', error: error.message });
  }
};
