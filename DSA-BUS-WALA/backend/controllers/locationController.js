...
const pushLocationUpdate = async (tripId, { lat, lng, speed, heading, timestamp }) => {
  ...
  // Persist the canonical normalized breadcrumb to the time-series collection
  // AND enqueue the same record to the background Excel exporter. The
  // deterministic eventId prevents duplicates on API retries.
  try {
    const trip = await Trip.findById(tripId, 'bus route').lean();
    if (trip) {
      await persistGpsRecord(
        {
          busId: trip.bus?.toString(),
        ...
        {
          routeId: trip.route?.toString() || null,
          busId: trip.bus?.toString() || null,
          tripId: trip._id?.toString() || null,
          source: 'driver-socket',
          apiStatus: 'OK'
        }
      );
    }
  } catch (locErr) {
    logger.error('LocationEvent Write Error:', locErr.message);
  }
...
const handleDriverLocationUpdate = async (io, socket, payload) => {
  ...
  // Track once-only "2 stops away" and "1 stop away" alerts per student per trip.
  if (!state.stopAwayAlerts) {
    state.stopAwayAlerts = new Map(); // studentId -> Set of {2,1}
  }
  ...
  const checkPush = async () => {
    try {
      ...
      for (const assignment of assignments) {
        ...
        // Compute how many stops away the bus is from the student's stop (using shared helper).
        const currentAway = stopsAway(state.routeStops, state.currentStopIndex || 0, assignment.stop?.sequence);

        // ── Once-only "N stops away" alerts (2 and 1) ──
        const studentAlerts = state.stopAwayAlerts.get(student._id.toString()) || new Set();
        if (currentAway === 2 && !studentAlerts.has(2)) {
          await sendPush(student, {
            title: '🚍 Bus is 2 stops away!',
            body: `Your bus is 2 stops from ${assignment.stop?.name || 'your stop'}. Get ready!`,
            url: '/student',
            tag: 'proximity-alert',
        ...
        if (currentAway === 1 && !studentAlerts.has(1)) {
          await sendPush(student, {
            title: '🚍 Bus is 1 stop away!',
            body: `Your bus is 1 stop from ${assignment.stop?.name || 'your stop'}. Almost there!`,
            url: '/student',
            tag: 'proximity-alert',
            icon: '/logo%20vertical.png',
            badge: '/favicons/android-chrome-192x192.png'
        ...
        if (studentAlerts.size) {
          state.stopAwayAlerts.set(student._id.toString(), studentAlerts);
        }

        if (withinMeters || withinMinutes) {
          console.log(`[Proximity] Alert to ${student.name}: ${etaMinutes} min (${Math.round(dist)}m)`);

          const stopsText = (currentAway != null && currentAway > 0)
            ? ` — ${currentAway} stop${currentAway > 1 ? 's' : ''} away`
            : '';

          await sendPush(student, {
            title: '🚌 Bus Arriving Soon!',
            body: `Bus is ${etaMinutes} min away (${Math.round(dist)}m${stopsText}). Get ready!`,
            url: '/student',
            tag: 'proximity-alert',
            icon: '/logo%20vertical.png',
            badge: '/favicons/android-chrome-192x192.png'
...
module.exports = { registerLocationHandlers, handleDriverLocationUpdate };
